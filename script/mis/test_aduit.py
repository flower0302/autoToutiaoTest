...
# 其次:其实对于测试用例封装无非试讲手工形式操作以及代码脚本的形式进行重新,那我的思路是这样的:
# 在一个测试类中,我们可能会实现多个测试方法,例如一个功能模块的多个测试点或者一个测试点的多种测试数据
# 为了模拟真实的手机测试情况,我们只需要打开一次浏览器,引入pytest单元测试框架中fixture的
# 一种思想,可以实现整个测试用例只会打开一次浏览器和关闭一次浏览器.
# 在fixture使用时类级别的fixture,在里面的会通过调用封装好的一个工具类中获取浏览器驱动的方法来打开一个
...
class TestAduit:
    ...
    # 定义测试方法, 在测试方法中调用已经封装好po文件中业务层中提供的业务方法来实现具体的测试操作
